...
description: "Improve a recurring workflow with baseline metrics, root causes, and a small experiment—not a binder no one reads."
doc_status: draft
tags:
  - playbook
---
...
Improve a recurring workflow with baseline metrics, root causes, and a small experiment—not a binder no one reads.

## When to use

- Repeated failures or thrash in a workflow.
- Team agrees the process is the bottleneck.

## Inputs required

- Process map or ticket samples
- Baseline time/cost/error rate (rough ok)
...
```text
You are assisting Innerflect on: Process improvement.
...
Task:
Execute "Process improvement" for the user's context. If the request spans multiple playbooks, say which playbook should own each part.

Category: operations
Playbook file: process-improvement.md
```
...
- [Retrospective](../project-management/retrospective.md)
