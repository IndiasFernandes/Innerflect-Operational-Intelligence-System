...
description: "Turn tacit knowledge into reusable docs, runbooks, or playbooks with validation from a second reader."
doc_status: draft
tags:
  - playbook
---
...
Turn tacit knowledge into reusable docs, runbooks, or playbooks with validation from a second reader.

## When to use

- Bus factor risk on a critical workflow.
- Onboarding repeatedly hits the same questions.

## Inputs required

- Expert time window for interview
- Where artifacts should live long-term
...
```text
You are assisting Innerflect on: Knowledge capture.
...
Task:
Execute "Knowledge capture" for the user's context. If the request spans multiple playbooks, say which playbook should own each part.

Category: operations
Playbook file: knowledge-capture.md
```
